...
type alias Event =
    { name : String, order : Int, answer : String }
...
type EnigmaState
    = FirstTry (List Event)
    | SecondTry (List Event) (List Event)
    | ThirdTry (List Event) (List Event) (List Event)
    | Result (List Event)


type Model
    = FirstQuestion { enigma : Enigma, state : EnigmaState }
    | OtherQuestions { previousResults : Nonempty ( Enigma, Int ), current : Enigma, currentState : EnigmaState, remaining : List Enigma }
    | Done (List ( Enigma, Bool ))


init : Model
init =
    FirstQuestion { state = FirstTry firstEnigma.events, enigma = firstEnigma }


firstEnigma : Enigma
firstEnigma =
    { events = [ { name = "Invention du Père Noël", order = 1, answer = "1392" } ] }
